...
function Cart() {
  const router = useRouter();
    ...
    const delteItem = (id)=>{
        CartApis.deleteCartItem(id).then(Response=>{
            if(Response)
    ...
    let total = 0 ;
    cart.map(item=>{
        total = total +item.product?.attributes?.Price ;
        })
    return total ;
    }
  return (
<section>
 ...
 <div>
       <h1 className="font-bold text-xl mx-2">${item.product?.attributes?.Price}</h1>
     </div>

   <button onClick={()=>delteItem(item?.id)} className="text-gray-600 transition hover:text-red-600">
